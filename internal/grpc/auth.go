...
type Auth interface {
	...
	Get(context.Context, int64) (int64, string, string, models.UserRole, *timestamppb.Timestamp, *timestamppb.Timestamp, error)
...
func (s *serverApi) Get(ctx context.Context, req *v1.GetRequest) (*v1.GetResponse, error) {
	id, name, email, role, createdAt, updatedAt, err := s.auth.Get(ctx, req.Id)
	if err != nil {
		return nil, err
	}
